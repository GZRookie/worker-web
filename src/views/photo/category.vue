  ...
  <div class="category-container">
    <el-card>
      ...
      <el-table :data="categoryList" v-loading="loading" border>
        ...
        <!-- <el-table-column prop="count" label="照片数量" width="100" /> -->
...
export default {
  ...
  setup() {
      ...
      formRef.value.validate((valid) => {
        if (valid) {
          ...
          if(dialogType.value === 'add') {
            ...
            service.post("/admin/category/update", state.form).then((res) => {
                if(res.code === 200) {
    ...
    const handleDelete = (row) => {
      ElMessageBox.confirm(`确认要删除「${row.name}」分类吗？`, '警告', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        // 模拟删除请求
       service.delete("/admin/category/delete/"+row.id).then((res) => {
            if(res.code === 200) {
    ...
    const getInitList = () => {
      loading.value = true
      service.post("/admin/category/page", state.queryParams).then((res) => {
        console.log(res)
        if(res.code === 200) {
